...
if ($item['type'] == 'file') {
	if ($item['url']) {
		echo CHtml::link(
			CHtml::image(Yii::app()->getBaseUrl(false)."/uploads/advertising/{$item['file_path']}", $item['alt_text']),
			$item['url'],
			array(
				'target' => '_blank',
		...
		echo CHtml::image(
			Yii::app()->getBaseUrl(false)."/uploads/advertising/{$item['file_path']}", $item['alt_text']
		);
	}
}
