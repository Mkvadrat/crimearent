jQuery(function ($) {
     ...
     */
    $('.searchField').live('change selectmenuchange', function (event) {
        changeSearch();
    });
});
...
function reloadApartmentList(url) {
    $.ajax({
        ...
        success: function (msg) {
            ...
            if ($('div.main-content-wrapper').length>0) {
                $('div.main-content-wrapper').html(msg);
            } else if ($('div#appartments-list-wrapper').length>0) {
                $('div#appartments-list-wrapper').html(msg);
            } else if ($('div.appartments-list-container').length>0) {
                $('div.appartments-list-container').html(msg);
            }
...
//		}
        if($("#"+resultBlock).offset().top > $(document).height()-$(window).height()){
            dest=$(document).height()-$(window).height();
        }else{
            dest=$("#"+resultBlock).offset().top;
        }
    ...
    if (!searchLock) {
        ...
        $.ajax({
            ...
            success: function (data) {
                try {
                    $('#btnleft').attr('value',data.string);
