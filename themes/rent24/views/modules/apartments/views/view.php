...
<div class="print-ico-wrapper">
    <a class="print-ico" href="<?php echo $model->getUrl().'?printable=1' ?>" target="_blank">Распечатать объявление</a>
</div>
...
<div class='div-pdf-fix'>
	...
	<div class="floatleft-title">
        ...
        <div class="product-attributes">
            <?php if (isset($model->city) && !empty($model->city->name)): ?>
            <label>Город:</label>  <?php echo CHtml::encode($model->city->name) ?> <span class="sep">|</span>
            <?php endif ?>
            <?php $street = $model->getStrByLang('address') ?>
            <?php if (!empty($street)): ?>
                <?php $house = $model->getStrByLang('house') ?>
            ...
            <?php if ($model->is_price_poa)
                ...
                echo CHtml::encode($model->getPrettyPrice());
            ?>
            <?php if (isset($model->num_of_rooms)): ?>
                <span class="sep">|</span>
                <label>Комнат:</label>  <?php echo CHtml::encode($model->num_of_rooms) ?>
            <?php endif ?>
            <?php if (isset($model->berths)): ?>
                <span class="sep">|</span>
                <label>Мест:</label>  <?php echo CHtml::encode($model->berths) ?>
            <?php endif ?>
        </div>
	</div>
</div>
...
</div><!--/"container product-container" from inner_content_middle.php-->
...
//$img = CHtml::image($res['thumbUrl'], $res['comment'], array('class'=>'shadow-down'));
//if($res['link']){
...
//if($res['link']){
//    $img = CHtml::image($res['link'], $res['comment'], array('class'=>'shadow-down'));
//
//    echo CHtml::link($img, $res['link'], array(
//        'rel' => 'prettyPhoto[img-gallery]',
//        'title' => $res['comment'],
...
if($res['link']){
    $main_image = Images::getMainThumbImage($model->images);
    if ($main_image) {
        $_img = Images::getThumbUrlWatermark($main_image, 600, 400);
        $img = CHtml::image($_img, $res['comment'], array('class'=>'shadow-down'));
...
//            'title' => '',
//        ));
        echo CHtml::link($img, $_img, array(
            'rel' => 'prettyPhoto[img-gallery]',
            'title' => '',
        ));
    }
...
<div class="container product-container">
    ...
    <div class="panel-wrapper thumbs-wrapper"><div class="panel"><div class="panel-row">
    <?php
    if ($model->images) {
