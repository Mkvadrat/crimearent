...
class Apartment extends ParentModel
{
    ...
    const SUB_TYPE_NONE = 0;
    const SUB_TYPE_HOUR = 1;
    const SUB_TYPE_DAY = 2;
    const SUB_TYPE_WEEK = 3;
    const SUB_TYPE_MONTH = 4;

    CONST ENABLE_SUBTYPE = false;
    ...
    const MODERATION_VIEW_TIME = 259200; // 3 дня
    const SHOW_MODERATION_OBJECTS_IN_TIME = true;
    ...
    public function rules()
    {
        ...
        $rules = array(
            ...
            array('subtype', 'numerical', 'integerOnly' => true),
            array('sea_distance', 'numerical', 'integerOnly' => false),
            array('berths_max', 'numerical', 'integerOnly' => true),
            array('count_img', 'checkPhotos')
        );
    ...
    public function checkPhotos() {
        if ($this->type == Apartment::TYPE_BUY || $this->type == Apartment::TYPE_RENTING) return;

        if (empty($this->count_img)) {
            $this->addError('count_img', 'Нужно загрузить фотограцию');
        }
    }
    ...
    public function checkPhone2() {
        if (!empty($this->phone2)) {
            $this->phone2 = str_replace(' ','',$this->phone2);
            $this->phone2 = str_replace('-','',$this->phone2);
            if (!preg_match('/^[+][\d]{11,}$/',$this->phone2) && !preg_match('/^[8][\d]{10,}$/',$this->phone2)) {
                $this->addError('phone2', 'Телефон должен быть в международном формате');
            }
        }
    }
    ...
    public function attributeLabels()
    {
        return array(
            ...
            'rent_type' => tt('Rent Type', 'apartments'),
            'sea_distance' => tt('Sea distance', 'apartments'),
            'berths_max' => tt('Maximum number of berths', 'apartments'),
            'house' => tt('House', 'apartments'),
            'phone2' => tt('Second phone', 'apartments'),
            'count_views' => tt('Views count', 'apartments'),
        );
    }
    ...
    public function beforeDelete()
    {
        ...
        ApartmentBanner::remove($this->id);
    ...
    public function getPrettyPrice()
    {
        if (Yii::app()->theme->name == 'atlas') {
            ...
            $data = array(
                '{CURRENCY}' => $this->getCurrency(),
                '{PRICE}' => $this->setPretty($price),
                '{TYPE}' => self::getPriceName($this->price_type),
            );
            ...
            if ($this->isPriceFromTo()) {
                ...
                if ($price)
                    ...
                    $priceFromTo = tc('price_from') . ' ' . $price . ' ' . $this->getCurrency();
                if ($priceTo)
                    //$priceFromTo .= $priceTo ? ' ' . tc('price_to') . ' ' . $this->setPretty($priceTo) . ' ' . $this->getCurrency() : '';
                    $priceFromTo .= $priceTo ? ' ' . tc('price_to') . ' ' . $priceTo . ' ' . $this->getCurrency() : '';

                return $priceFromTo;
            }

            //return $this->setPretty($price) . ' ' . $this->getCurrency() . ' ' . self::getPriceName($this->price_type);
            return $price . ' ' . $this->getCurrency() . ' ' . self::getPriceName($this->price_type);
        }
    }
    ...
    public static function getAvailableSubTypeIds() {
        return array(
            self::SUB_TYPE_HOUR,
            self::SUB_TYPE_DAY,
            self::SUB_TYPE_WEEK,
    ...
    public static function getSubTypesArray() {
        return array(
            self::SUB_TYPE_NONE => tt('Please choose', 'apartments'),
            self::SUB_TYPE_HOUR => tt('Rent per hour', 'apartments'),
            self::SUB_TYPE_DAY => tt('Rent per day', 'apartments'),
            self::SUB_TYPE_WEEK => tt('Rent per week', 'apartments'),
            self::SUB_TYPE_MONTH => tt('Rent per month', 'apartments')
        );
    }

    public static function getSeaDistanceMinMax($objTypeId = 1, $all = false)
    {
        $ownerActiveCond = '';
        if (param('useUserads'))
            $ownerActiveCond = ' AND owner_active = ' . self::STATUS_ACTIVE . ' ';
        ...
        if ($all)
            $sql = 'SELECT MIN(sea_distance) as sea_min, MAX(sea_distance) as sea_max FROM {{apartment}} WHERE type IN (' . implode(',', Apartment::availableApTypesIds()) . ') AND active = ' . self::STATUS_ACTIVE . ' ' . $ownerActiveCond;
        else
            $sql = 'SELECT MIN(sea_distance) as sea_min, MAX(sea_distance) as sea_max FROM {{apartment}} WHERE type IN (' . implode(',', Apartment::availableApTypesIds()) . ') AND obj_type_id = "' . $objTypeId . '" AND active = ' . self::STATUS_ACTIVE . ' ' . $ownerActiveCond;
