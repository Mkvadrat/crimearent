...
class Images extends ParentModel {
    ...
    public static function createThumbWatermark($image, $width, $height) {
        $newPath = self::returnModifiedThumbPath($image, $width, $height);
        $originalPath = self::returnOrigPath($image);
        if(!file_exists($originalPath)){
            return '';
    ...
    public static function getThumbUrlWatermark($image, $width=0, $height=0){
        if($image['file_name_modified']){
            $modifiedFile = self::returnModifiedThumbPath($image, $width, $height);
            ...
            if(file_exists($modifiedFile)){
                ...
                return self::createThumbWatermark($image, $width, $height);
            }
        } else {
            $image['file_name_modified'] = self::updateModifiedName($image);
            return self::createThumbWatermark($image, $width, $height);
	...
	public static function getMainThumb($width, $height, $images, $id = null, $default_image_url = null){
		$return = array(
			'link' => '',
			'thumbUrl' => '',
			'comment' => '',
		...
		} else if (!empty($default_image_url)) {
            $return['thumbUrl'] = $default_image_url;
        } else {
    ...
    public static function getMainThumbImage($images){
        $image = null;
        if($images){
            foreach($images as $img){
                if($img['is_main']){
